...
from app.validators import verify_request_data_format, verify_required_params, verify_params_length, verify_email_param_suffix, authentication_required
from app.search import search_email, export_all_users
...
@verify_request_data_format
@verify_required_params({'email': str, 'message': str})
...
def messages_send(data):
    if data is not None:
        ret_member_id, ret_member_info = search_email(data['email'])
        if ret_member_id and ret_member_info:
            ret_send_status, ret_send_result = send_message(ret_member_id, data['message'])
